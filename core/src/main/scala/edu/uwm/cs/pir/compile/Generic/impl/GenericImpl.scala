...
object GenericImpl {
  ...
  class InvertedIndexSearcher(val index: InvertedIndex, val tokenizer: Tokenizer) {
    ...
    def searchOR(q: String, topk: Int) = {
      ...
      accums.map(d => InvertedIndexSearchResult(d._1, d._2.docStringId, index.dataset(d._1), d._2.score / docNorm(d._1))).toSeq.sortWith((elem1, elem2) => (elem1 > elem2)).take(topk)
    }
  }
