...
//    }
    ...
    def connect[NewOut <: IFeature, Indexer <: IIndexer, Index <: IIndex, Compose <: ICompose]
    (query: GenericLuceneQuery, index: IndexStage[Out, Index], compose: GenericCompose[Out, Compose]) = {
...
//    }
  ...
  class SourcePipe[In <: IFeature, Out <: IFeature : ClassManifest]
                  (val left: SourceComponent[In], val right: ProjComponent[In, Out]) 
