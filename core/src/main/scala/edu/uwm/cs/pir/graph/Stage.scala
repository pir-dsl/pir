...
object Stage {
  ...
  @SerialVersionUID(1L)
  class ProjWithModelStage[In <: IFeature, Out <: IFeature, Model <: IModel](override val proj: GenericProjWithModel[In, Out, Model],
  ...
  @SerialVersionUID(1L)
  class IndexStage[In <: IFeature, Index <: IIndex](val indexer: GenericIndex[In, Index], val source: List[SourceComponent[In]]) extends Vertex with Serializable {
  ...
  @SerialVersionUID(1L)
  class HistogramIndexStage[In <: IFeature, Index <: IIndex](val indexer: GenericBasicIndex[In, Index], val source: SourceComponent[In])(implicit c: ClassTag[Index])
    extends Vertex with Serializable {

    var isDirty = false
    var cacheIndex: Option[Index] = None
    var cacheRDDIndex: Option[RDD[Index]] = None

    def setIndex(index: Option[Index]): Unit = {
      cacheIndex = index
    }
    
    def setRDDIndex(index: Option[RDD[Index]]): Unit = {
      cacheRDDIndex = index
    }
  ...
  class NaiveIndexQueryStage[In <: IFeature, Out <: IFeature, Index <: IIndex](val query: GenericProjWithBasicIndex[In, Out, Index],
    ...
    var isDirty = false
    var cacheQueryResult: Option[Index] = None
  ...
  @SerialVersionUID(1L)
  class LuceneQueryStage[In <: IFeature, Out <: IFeature, Index <: IIndex, Compose <: ICompose](val query: GenericProjWithIndex[In, Out, Index],
    val source: SourceComponent[In],
